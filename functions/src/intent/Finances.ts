...
export class Finances {
  ...
  static onPaymentWebhook = functions.https.onRequest((req, res) => {
    ...
    if (req.method === "OPTIONS") {
      ...
      rtdb
        .ref(`temp/transactions/${transaction_uid}`)
                ...
                switch (`${transaction_type}`.toLowerCase()) {
                  case "payment":
                    moneyAccountDoc
                      ...
                      .then(ignored => {
                        rtdb
                          .ref("z-platform/statistics/finances")
                          .once("value", globalAccountSnapshot => {
                            if (globalAccountSnapshot.exists) {
                              const {
                                platform_balance
                              } = globalAccountSnapshot.val();
                              globalAccountSnapshot.ref.set({
                                platform_balance:
                                  +transaction_amount + platform_balance
                              });
                            } else {
                              globalAccountSnapshot.ref.set({
                                platform_balance: +transaction_amount
                              });
                            }
                          });
                        ...
                        rtdb
                          .ref("intents/notification/deposit")
                          .push({
  ...
  static onPayment = functions.database
    .ref("/intents/pay/{today_midnight}/{ref}")
    .onCreate((snapshot, context) => {
      ...
      const formData = {
        ...
        transaction_receiver_webhook_url: functions.config().wecashup
          .payment_webhook,
        transaction_receiver_currency: "XAF",
        transaction_receiver_total_amount: +data.amount,
      ...
      return rp(options)
        .then(function(body) {
          ...
          const {
            transaction_token,
            transaction_uid
          } = bodyData.response_content;
          snapshot.ref
            .child("api_generated_details")
            .set({ transaction_token, transaction_uid });

          return snapshot.ref.child("response").set({
            code: 200,
  ...
  static onWithdraw = functions.database
    .ref("/intents/withdraw/{today_midnight}/{ref}")
    .onCreate((snapshot, context) => {
      /* Wecashup accepts request with content type set to url-encoded */
      const data = snapshot.val();
      console.log({ data });
      const formData = {
        ...
        transaction_product_data: {
          product_1: {
            ...
            category: "withdrawal",
            description:
              "withdrawal of money from ezykargo account to a mobile money account"
          }
        }
      };
      ...
      return rp(options)
        .then(function(body) {
          ...
          admin
            ...
            .set({
              ...bodyData.response_content.transaction,
  ...
  static putMoneyInEscrow = async (
    amountToBePutInEscrow,
    referenceString,
    userRefString
  ) => {
    ...
    const accountSnapshot = await firestore
      .doc(`/bucket/moneyAccount/moneyAccounts/${userRef.id}`)
      .get();
    if (accountSnapshot.exists) {
      const data = accountSnapshot.data();
      const { balance, escrowTotal } = data;
      if (
        balance >= escrowTotal + amountToBePutInEscrow &&
        amountToBePutInEscrow > 0
      ) {
        ...
        batch.update(accountSnapshot.ref, {
          escrowTotal: escrowTotal + amountToBePutInEscrow
        });

        return batch.commit().then(ignored => {
          return {
            message: "ok",
            successStatus: true
          };
  ...
  static removeMoneyFromEscrow = async (referenceString, userRefString) => {
    ...
    if (accountSnapshot.exists) {
      ...
      return firestore
        ...
        .then(escrowSnapshot => {
          console.log({
            escrow: escrowSnapshot.data(),
            escrowRef: escrowSnapshot.ref.path
          });
          ...
          return accountSnapshot.ref
            ...
            .then(ignored => {
              return escrowSnapshot.ref.delete().then(ignored2 => {
                return { successStatus: true, message: "ok" };
              });
            });
        });
    }
  ...
  static payMoneyInEscrow = functions.database
    .ref("/intents/freightage_complete/{timestamp}/{push_id}")
    .onCreate(async (snapshot, context) => {
      ...
      if (!escrowSnapshot.exists) {
        return Promise.reject(
          "The escrow in question doesn't exist. Seems like he overpressed a button"
        );
      }
      ...
      const driverData = drivers.map(driverDatum => {
        const totalWeight = driverDatum.items.reduce((acc, curItem) => {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        if (
          !is_active &&
          ezyBizReferrerRefString !== Constants.EZYKARGO_REFERRER
        ) {
          await firestore
            .doc(ezyBizReferrerRefString)
            .get()
            .then(async referrerMoneyAccountSnapshot => {
              if (referrerMoneyAccountSnapshot.exists) {
                const {
                  active_referred_ones_count
                } = referrerMoneyAccountSnapshot.data();
        ...
        if (
          ezyBizReferrerRefString !== Constants.EZYKARGO_REFERRER &&
          ezyBizReferralCommissionCount < Constants.MAX_REFERRAL_COMMISSIONS
        ) {
          promises.push(
        ...
        driverData.forEach(async driverDatum => {
          ...
          const driverMoneyAccountRef = firestore.doc(
            `/bucket/moneyAccount/moneyAccounts/${driverRef.id}`
          );
          ...
          const driver_data: any = driverAndMoneysnapshot.reduce(
            (acc, tempSnapshot) => ({ ...acc, ...tempSnapshot.data() }),
            {}
          );
           ...
           */
          if (
            Boolean(driverRefererRefString) &&
            driverRefererRefString !== Constants.EZYKARGO_REFERRER &&
            !isDriverActive
          ) {
            firestore
              .doc(driverRefererRefString)
              .get()
              .then(async referrerMoneyAccountSnapshot => {
                if (referrerMoneyAccountSnapshot.exists) {
                  const {
          ...
          if (
            driverRefererRefString !== Constants.EZYKARGO_REFERRER &&
            driverReferralComissionCount < Constants.MAX_REFERRAL_COMMISSIONS
          ) {
            subPromises.push(
              Referrals.cutReferralCommission(
                `/bucket/usersList/users/${driverRef.id}`
              )
            );
          }

          const driverPriceToBePaidShare =
            (driverDatum.priceToBePaid * truck.percentage) / 100;
          subPromises.push(
            firestore
              .runTransaction(t => {
                return t
                  .get(driverMoneyAccountRef)
                  .then(driverMoneyAccountSnapshot => {
                    const {
                      balance: driverMoneyAccountBalance
                    } = driverMoneyAccountSnapshot.data();
                    const driverNewBalance =
                      driverMoneyAccountBalance + driverPriceToBePaidShare;
                    t.update(driverMoneyAccountRef, {
              ...
              .then(({ driverNewBalance, amountPaid, previousBalance }) => {
                const prom1 = driverMoneyAccountRef
                  .collection("transactions")
                  .add({
                    type: "pay_in",
                    amount: amountPaid,
                ...
                const prom2 = firestore
                  .doc(`bucket/usersList/users/${driverMoneyAccountRef.id}`)
                  .get()
                  .then(async driverSnapshot => {
                    const {
                      phonenumber: truckerPhonenumber,
                      fullName: truckerFullName,
                      truck: truckerTruck,
                      avatarUrl
                    } = driverSnapshot.data();
                     ...
                     */
                    await firestore
                      .collection(`${truckerTruck.truckRef}/transactions`)
                      .add({
                        type: "pay_in",
                        amount: amountPaid,
                        timestamp: FieldValue.serverTimestamp(),
                        driverRef: driverSnapshot.ref,
                        driverName: truckerFullName,
                        finished: true,
                        avatarUrl
                      });
                    ...
                    const driverMessagePaymentIntent = {
                      ...
                      to: PhonenumberUtils.parsePhonenumberForMessaging(
                        truckerPhonenumber
                      ),
                      data: {
                        name: title,
                    ...
                    console.log({
                      ezyBizMessagePaymentIntent: driverMessagePaymentIntent
                    });
                    return database
                      .ref(`/intents/message/${context.params.timestamp}`)
          ...
          const ownerMoneyAccountRef = firestore.doc(
            `/bucket/moneyAccount/moneyAccounts/${ownerRef.id}`
          );
          const ownerPriceToBePaidShare =
            driverDatum.priceToBePaid * ((100 - truck.percentage) / 100);
          subPromises.push(
            firestore
              .runTransaction(t => {
                return t
                  .get(ownerMoneyAccountRef)
                  .then(async ownerMoneyAccountSnapshot => {
                    ...
                    t.update(ownerMoneyAccountSnapshot.ref, {
                      is_active: true,
                      balance:
                        ownerMoneyAccountBalance + ownerPriceToBePaidShare
                    });
                    return {
              ...
              .then(
                ({
                  ...
                  const prom1 = ownerMoneyAccountRef
                    .collection("transactions")
                    .add({
                      type: "pay_in",
                      amount: ownerPriceToBePaidShare,
                  ...
                  const prom2 = firestore
                    .doc(`bucket/usersList/users/${ownerRef.id}`)
                    .get()
                    .then(ownerSnapshot => {
                      const {
                      ...
                      console.log({
                        ezyBizMessagePaymentIntent: ownerMessagePaymentIntent
                      });
                      return database
                        .ref(`/intents/message/${context.params.timestamp}`)
          ...
          return Promise.all(subPromises).then(ignored => escrowRef.delete());
        });
         ...
         */
        firestore
          .doc(`bucket/usersList/users/${ezyBizMoneyAccountSnapshot.id}`)
          .get()
          .then(ezybizSnapshot => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      accountRef
        ...
        .once("value", pendingTransactionsListSnapshot => {
          ...
          promises.push(
            accountRef.child("balance").transaction(financesAccount => {
              if (financesAccount) {
                ...
                return {
                  ...financesAccount,
                  ezykargo_balance: platformBalanceAfter
                };
              } else {
          ...
          const docRef = firestoreDb
            .collection("bucket/finances/savedTransactions/")
            .doc();
          promises.push(
            docRef.set({
          ...
          Promise.all(promises)
            ...
            .catch(err => {
              ...
              rtdb
                .ref("z-platform/statistics/errors")
                .push({
