...
export class Finances {
  ...
  static onTransactionCode = functions.database
    .ref("/intents/pay/{today_midnight}/{ref}/api_generated_details")
    .onUpdate((change, context) => {
      ...
      if (change.before.val().transaction_provider_code) {
        ...
        const options = {
          ...
          uri: `https://www.wecashup.com/api/v2.0/merchants/${
            functions.config().wecashup.merchant_uid
          }/transactions/${data.transaction_uid}?merchant_secret=${
            functions.config().wecashup.merchant_secret_key
          }&merchant_public_key=${
            functions.config().wecashup.merchant_public_key
          }`,
          form: { ...formData }
        };
        ...
        return rp(options)
          .then(function(body) {
            ...
            if (parseInt(bodyData.response_code) === 201) {
              return admin
                .database()
                .ref(`temp/transactions/${data.transaction_uid}`)
                .set({
                  ...bodyData.response_content,
                  timestamp: FieldValue.serverTimestamp()
                });
          ...
          .catch(err => {
            ...
            return admin
              ...
              .then(ignored => {
                change.after.ref.child("response").set({ code: 500 });
              });
          });
      }
    });
    ...
    .onCreate(async (snapshot, context) => {
      ...
      if (escrowSnapshot.exists) {
        ...
        const driverData = drivers.map(driverDatum => {
          ...
          const priceToBePaid =
            (totalWeight * frieghtageRequestPrice) /
            Finances.TON_TO_KILO_DIVISOR;
          return {
            priceToBePaid,
        ...
        return new Promise(async (resolve, reject) => {
          ...
          driverData.map(driverDatum => {
            ...
            promises.push(
              driverRef.get().then(driverSnasphot => {
                const subPromises = [];
                const {
                ...
                if (driverRefererRefString !== Finances.EZYKARGO_REFERRER) {
                  subPromises.push(
                    Referrals.cutReferralCommission(
                      `/bucket/usersList/users/${driverSnasphot.ref.id}`
                    )
                  );
                }
                const driverMoneyAccountRef = firestore.doc(
                  `/bucket/moneyAccount/moneyAccounts/${driverSnasphot.ref.id}`
                );
                const driverPriceToBePaidShare =
                  (driverData.priceToBePaid * truck.percentage) / 100;
                subPromises.push(
                ...
                subPromises.push(
                  driverMoneyAccountRef.collection("transactions").add({
                    type: "payment",
                ...
                const ownerPriceToBePaidShare =
                  (driverData.priceToBePaid * (100 - truck.percentage)) / 100;
                subPromises.push(
                  firestore
                    .runTransaction(t => {
                      return t
                    ...
                    .then(ezyownerReferrerRefString => {
                      if (
                        driverRefererRefString !== Finances.EZYKARGO_REFERRER
                      ) {
                ...
                subPromises.push(
                  ownerMoneyAccountRef.collection("transactions").add({
                    type: "payment",
          ...
          const ezyKargoPlatformEzybizCommission =
            amount_in_escrow - frieghtageRequestPrice;
          promises.push(
            Finances.addEzyKargoPlatformTransaction(
              ezyKargoPlatformEzybizCommission,
              `firestoreRef : ${freightageSnapshot.ref}`
            )
          );
    ...
    if (req.method === "OPTIONS") {
      ...
      accountRef
        ...
        .once("value", pendingTransactionsListSnapshot => {
          ...
          promises.push(
            accountRef.transaction(financesAccount => {
              if (financesAccount) {
