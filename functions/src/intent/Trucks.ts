...
export class TrucksIntent {
  ...
  static listenDeleteTruckIntent = functions.database
    .ref("/intents/delete_truck/{timestamp}/{ref}")
    .onCreate((snapshot, context) => {
      ...
      return (
        firestore
          .doc(truckDataSnapshot.truckRefString)
          .get()
          ...
          .then(truckSnapshot => {
            ...
            if (truckData.hasCurrentDriver) {
              snapshot.ref
                .child("response")
                .set({ code: TrucksIntent.INCONSISTENT });
              return false;
            }
            if (truckSnapshot.get("userRef") === truckDataSnapshot.userRef) {
              return truckSnapshot.ref
                ...
                .then(() => {
                  ...
                  if (driver) {
                    firestore
                      .doc(truckDataSnapshot.driver.ref)
  ...
  /*
    ...
    */
  static listenLinkNewDriverTruckIntent = functions.database
    .ref("/intents/associate_driver/{timestamp}/{push_id}")
    .onCreate((snapshot, context) => {
        ...
        .then(async truckSnapshot => {
          ...
          if (newIntentDataSnapshot.driverRef != "N/A") {
            await new Promise((resolve, reject) => {
              admin
                .database()
                .ref(newIntentDataSnapshot.consentRef)
                .once("value", consentSnapshot => {
            ...
            if (
              consentData.response.code !== 200 &&
              consentData.response.driverRef !== newIntentDataSnapshot.driverRef
            ) {
              snapshot.ref.child("response").set({ code: 401 });
              return false;
            }
          ...
          if (truckSnapshot.get("userRef") === newIntentDataSnapshot.userRef) {
            ...
            promises.push(
              firestore
                .collection(truckSnapshot.ref.path + "/drivers")
            ...
            //add truck info to driver
            promises.push(
              firestore.doc(newIntentDataSnapshot.driverRef).set(
                {
                  truck: {
                    contractDate: FieldValue.serverTimestamp(),
                    percentage: consentData.percentage,
  ...
  /*
    ...
    */
  static listenUnLinkDriverTruckIntent = functions.database
    .ref("/intents/dissociate_driver/{timestamp}/{push_id}")
    .onCreate((snapshot, context) => {
      ...
      return Trucks.getDocByRef(dissociateDriverDataSnapshot.truckRef)
        .then(truckSnapshot => {
          ...
          return firestore
            .doc(dissociateDriverDataSnapshot.driverRef)
            .get()
            .then(driverSnapShot => {
              if (
                dissociateDriverDataSnapshot.driverRef !== "N/A" &&
              ...
              if (
                truckSnapshot.get("userRef") ===
                dissociateDriverDataSnapshot.userRef
              ) {
                const promises = [];
                promises.push(
                  firestore
                    ...
                    .then(truckerDriverQuerySnapshot => {
                      truckerDriverQuerySnapshot.forEach(
                        truckDriverSnapshot => {
                          promises.push(
                            truckDriverSnapshot.ref.set(
                              {
                                idle: true,
                                truck: null,
                ...
                promises.push(
                  firestore
                    .collection(driverSnapShot.ref.path + "/notifications")
                    .orderBy("createdAt", "asc")
                    .where(
                      "action",
                      "==",
                      NotificationActions.DISSOCIATION_NOTIFICATION_ACTION
                    )
                    .limit(1)
                    .get()
                    .then(dissociationNotificationsSnapshot => {
                      dissociationNotificationsSnapshot.forEach(
                        dissociationNotificationSnapshot => {
                          promises.push(
                            dissociationNotificationSnapshot.ref.delete()
                          );
                        }
                      );
                    })
  ...
  /*
    ...
    - return code 201 or any appropriate error-code
    */
  static listenNotifyDriverOfDissociation = functions.database
    .ref("/intents/notify_dissociate_driver/{timestamp}/{push_id}")
    .onCreate((snapshot, context) => {
      const dissociateDriverDataSnapshot = snapshot.val();
      const firestore = admin.firestore();
      return Trucks.getDocByRef(dissociateDriverDataSnapshot.truckRef)
                  ...
                  firestore
                    ...
                    .then(driverQuerySnapshot => {
                      driverQuerySnapshot.forEach(truckerDriverSnapshot => {
                        console.log({
                          driverSnapshot: truckerDriverSnapshot.data()
                        });
                        promises.push(
                          truckSnapshot.ref.set(
                            {
  ...
  /*
    ...
    - return code 201 or any appropriate error-code
    */
  static listenCancelNotifyDriverOfDissociation = functions.database
    .ref("/intents/cancel_notify_dissociate_driver/{timestamp}/{push_id}")
    .onCreate((snapshot, context) => {
      const dissociateDriverDataSnapshot = snapshot.val();
      const firestore = admin.firestore();
      return Trucks.getDocByRef(dissociateDriverDataSnapshot.truckRef)
                  ...
                  firestore
                    ...
                    .then(driverQuerySnapshot => {
                      driverQuerySnapshot.forEach(truckerDriverSnapshot => {
                        ...
                        promises.push(truckSnapshot.ref.set({ ...truckData }));
                        promises.push(
                          truckerDriverSnapshot.ref.set(
                            {
                              markedForDissociation: false,
  ...
  /*
    ...
    - return code 201 or any appropriate error-code
    */
  static listenAddTruckIntent = functions.database
    .ref("/intents/add_truck/{timestamp}/{ref}/finished")
    .onCreate((snapshot, context) => {
      ...
      return new Promise((outerPromiseResolve, outerPromiseReject) => {
        db.ref(`/intents/add_truck/${timestamp}/${ref}`).once(
          "value",
          truckDataSnapshot => {
            ...
            promises.push(
              new Promise((resolve, reject) => {
                const truckRefTesting = firestore
                  .collection("bucket/trucksList/trucks")
                  .doc();
                      ...
                      firestore.runTransaction(t => {
                        ...
                        return t.get(refTrucks).then(trucksListSnaphsot => {
                          ...
                          if (trucksListSnaphsot.exists) {
                            trucksCount =
                              +trucksListSnaphsot.data().trucksCount + 1;
  ...
  static listenAddTechnicalVisitIntent = functions.database
    .ref("/intents/add_technical_visit/{ref}/finished")
    .onCreate(async (snapshot, context) => {
      if (!snapshot.val()) return false;
      const ref = context.params.ref;

      const TechnicalVistSnapshot = await admin
        .database()
        .ref(`/intents/add_technical_visit/${ref}`)
        .once("value");
      const TechnicalVistData = TechnicalVistSnapshot.val();

      const truckSnapShot = await Trucks.getDocByRef(
        TechnicalVistData.truck_ref
      );
      ...
      if (!truckSnapShot.data())
        return admin
          ...
          .ref.set({ code: 404 });
      if (
        truckSnapShot.data().immatriculation.toLocaleLowerCase() !==
        TechnicalVistData.immatriculation.toLocaleLowerCase()
      )
        return admin
          .database()
          .ref(`/intents/add_technical_visit/${ref}/response`)
      ...
      promises.push(
        admin
          .firestore()
          .collection(truckDocumentsPath + "/technical_visits")
          .add({
            ...TechnicalVistDoc,
      ...
      if (
        truckSnapShot.data().hasCurrentDriver &&
        truckSnapShot.data().driver_ref
      ) {
        promises.push(
          Users.getDocByRef(truckSnapShot.data().driver_ref).then(
            driverSnapshot => {
              ...
              if (
                driverSnapshot.data().truck.hasValidRegistrationCertificate &&
                driverSnapshot.data().truck.hasAValidInsurrance
              ) {
                truckDataForDriver.hasValidTruck = true;
              }
              return driverSnapshot.ref.set(truckDataForDriver, {
                merge: true
              });
            }
          )
  ...
  static listenAddInsurranceIntent = functions.database
    .ref("/intents/add_insurrance/{ref}/finished")
    .onCreate(async (snapshot, context) => {
      if (!snapshot.val()) return false;
      const ref = context.params.ref;

      const insurranceSnapshot = await admin
        .database()
        .ref(`/intents/add_insurrance/${ref}`)
        .once("value");
      ...
      if (!truckSnapShot.data())
        return admin
          .database()
          .ref(`/intents/add_insurrance/${ref}/response`)
          .ref.set({ code: 404 });
      if (
        truckSnapShot.data().immatriculation.toLocaleLowerCase() !==
        insurranceData.immatriculation.toLocaleLowerCase()
      )
        return admin
          .database()
          .ref(`/intents/add_insurrance/${ref}/response`)
      ...
      promises.push(
        admin
          .firestore()
          .collection(truckDocumentsPath + "/insurrances")
          .add({
            ...insurranceDoc,
        ...
        promises.push(
          Users.getDocByRef(truckSnapShot.data().driver_ref).then(
            driverSnapshot => {
              ...
              if (
                driverSnapshot.data().truck.hasValidTechnicalVisit &&
                driverSnapshot.data().truck.hasValidRegistrationCertificate
              ) {
                truckDataForDriver.hasValidTruck = true;
              }
  ...
  static listenAddRegistrationCertificateIntent = functions.database
    .ref("/intents/add_registration_certificate/{ref}/finished")
    .onCreate(async (snapshot, context) => {
      if (!snapshot.val()) return false;
      const ref = context.params.ref;

      const registrationCertificateSnapshot = await admin
        .database()
        .ref(`/intents/add_registration_certificate/${ref}`)
        .once("value");
      const registrationCertificateData = registrationCertificateSnapshot.val();

      const truckSnapShot = await Trucks.getDocByRef(
        registrationCertificateData.truck_ref
      );
      const repSnapShot = await Users.getDocByRef(
        registrationCertificateData.user_ref
      );

      if (!truckSnapShot.data())
        return admin
          .database()
          .ref(`/intents/add_registration_certificate/${ref}/response`)
          .ref.set({ code: 404 });
      if (
        truckSnapShot.data().immatriculation.toLocaleLowerCase() !==
        registrationCertificateData.immatriculation.toLocaleLowerCase()
      )
        return admin
          .database()
          .ref(`/intents/add_registration_certificate/${ref}/response`)
      ...
      promises.push(
        admin
          .firestore()
          .collection(truckDocumentsPath + "/registration_certificates")
          .add({
