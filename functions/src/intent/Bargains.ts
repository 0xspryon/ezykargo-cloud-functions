...
export class BargainsIntent {
            ...
            firestore.doc(Freightages.getRef(freightageRef)).get()
                .then(async freightageDataSnapshot => {
                    ...
                    freightageDataSnapshot.ref.set({
                        ...
                        .then(() => {
                            const departure_date = freightageDataSnapshot.data().departure_date
                            realtimeDatabase
                                ...
                                .once("value", (cityvalue) => {
                                    ...
                                    const freightageData = freightageDataSnapshot.data()
                                    realtimeDatabase
                                        .ref(`/cities/${freightageData.addressFrom.mLocality}/${departure_date}/${freightageDataSnapshot.data().addressTo.mLocality}`).ref.set({
                                            number: 0,
                                            weight: 0
                                        })

                                    drivers.map((driver) => {
                                        admin.database().ref('/intents/notification/generic')
                                            .push({
                                                title_fr: `Hooray!!! Vous avez été sélectionné.`,
                                                title_en: `Hooray!!! You have been selected.`,
                                                message_en: `${freightageData.title} has selected you at ${driver.price}/Ton.`,
                                                french_en: `${freightageData.title} Vous a sélectionné à ${driver.price}/Tonne.`,
                                                data: {
                                                    type: 'hired',
    ...
    static listenAcceptHireRequest = functions.database.ref('/intents/{timestamp}/accepted_hired_request/{freightageRef}/{userRef}/')
        .onCreate(async (snapshot, context) => {
            const firestore = admin.firestore()
            const realtimeDatabase = admin.database()
                ...
                .then(freightageDataSnapshot => {
                    ...
                    drivers = drivers.map((driver) => {
                        if (Users.getRef(userRef).indexOf(driver.driverRef) !== -1) {
                            if (postData.accepted && postData.selectedBargain.indexOf(driver.uniqID) !== -1)
                                driver.idle = false
                            else
                                driver.idle = false
                        }
                        ...
                        freightageDataSnapshot.ref.set({
                            ...
                            .then(async () => {
                                ...
                                await Notifications.createGenericNotification(
                                    ...
                                    `All the selected drivers refused your request to hire them for ${freightageData.title}`,
                                    `Tous les chauffeurs sélectionnés ont refusé votre demande de location pour ${freightageData.title}`,
                                    {
