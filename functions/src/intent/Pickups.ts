...
export class PickupsIntent {
     ...
     * 5- send fcm to biz 
     */
    static listenMarkAsPickup = functions.database.ref('/intents/mark_as_pickup/{timestamp}/{freightageRef}/{userRef}')
        .onCreate(async (snapshot, context) => {
            ...
            await firestore.runTransaction(t => {
                return t.get(firestore.doc(Freightages.getRef(freightageRef)))
                    .then(freightageDataSnapshot => {
                        const freightageData = freightageDataSnapshot.data()
                        /** Step 2 */
                        const driverFound = freightageData.drivers.find((driver)=>Users.getRef(userRef).indexOf(driver.driverRef)!==-1)
                        if(!driverFound){
                            return realtimeDatabase.ref(`/intents/mark_as_pickup/${timestamp}/${freightageRef}/${userRef}/response`).ref
                                .set({ code: 403 })
                        }
                        if(driverFound.pickup || driverFound.items.length !== 0) 
                            return realtimeDatabase.ref(`/intents/mark_as_pickup/${timestamp}/${freightageRef}/${userRef}/response`).ref
                                .set({ code: 409 })
                        /** Step 3 */
                        ...
                        postData.items.forEach(item => {
                            ...
                            if(!remote_item.carrying_quantity) remote_item.carrying_quantity = 0
                            if(item.quantity>remote_item.quantity-remote_item.carrying_quantity){
                                quantity_mismatch = true
                            }
                            remote_item.carrying_quantity += item.quantity
                            carrying_items.push({...remote_item,carrying_quantity: item.quantity,index: item.index})
                        });
                        ...
                        return freightageDataSnapshot.ref.set({
                            ...
                            drivers: freightageData.drivers.map((driver)=>{
                                if(userRef.indexOf(driver.driverRef)!==-1){
        ...
        */
       static listenValidatePickup = functions.database.ref('/intents/validate_pickup/{timestamp}/{freightageRef}/{userRef}')
           .onCreate(async (snapshot, context) => {
               const firestore = admin.firestore()
               const realtimeDatabase = admin.database()
               ...
               await firestore.runTransaction(t => {
                   return t.get(firestore.doc(Freightages.getRef(freightageRef)))
                       .then(freightageDataSnapshot => {
                           const freightageData = freightageDataSnapshot.data()
                           /** Step 2 */
                            if(Users.getRef(userRef).indexOf(freightageData.userRef)===-1)
                                return realtimeDatabase.ref(`/intents/validate_pickup/${timestamp}/${freightageRef}/${userRef}/response`).ref
                                    .set({ code: 401 })
                           /** Step 3 */
                           const driverFound = freightageData.drivers.find((driver)=>postData.driverRef.indexOf(driver.driverRef)!==-1)
                           if(!driverFound){
                               return realtimeDatabase.ref(`/intents/validate_pickup/${timestamp}/${freightageRef}/${userRef}/response`).ref
                                   .set({ code: 403 })
                           }
                           ...
                           if(postData.validate){
                               onTransit = true
                               pickup = false
                               freightageData.items.forEach(item => {
                                   if(!item.carrying_quantity) pickup = true
                                   if(item.quantity - item.carrying_quantity>0) pickup = true
                               });
                           }
