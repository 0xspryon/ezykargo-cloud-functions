...
export class TransactionsIntent {
    ...
    static listenMakePayment = functions.database.ref('/intents/make_deposit/{timestamp}/{ref}')
        .onCreate(async (snapshot, context) => {
            ...
            firestore.doc(data["userRef"]).get()
                .then(userDataSnapshot => {
                    ...
                    axios.post(placePayment,requestData)
                        .then(response => {
                            ...
                            transactionRef.set({
                                ...
                                if(result.status==="REQUEST_ACCEPTED"){
                                    realtimeDatabase.ref(`/intents/make_deposit/${timestamp}/${ref}/response`).ref
                                        .set({
                                            code: 202,
                                            response: result 
                                        });
    ...
    static validatePayment(data){
        return new Promise((resolve,reject)=>{
            const firestore = admin.firestore()
            const realtimeDatabase = admin.database()
            firestore.doc(data.payment_ref).get()
                .then(transactionDataSnapshot => {
                    ...
                    firestore.doc(Transactions.getRefMoneyAccount(userId)).get()
                        .then(dataSnapshot => {
                            let account;
                            ...
                            if(data.data==="success"){
                                newVal = {prevAmount: +account["balance"], newAmount: account["balance"] +transactionData}
                            }
                            transactionDataSnapshot.ref.set({ ...data,  ...newVal }, { merge: true })
                                .then(() => {
                                    if(data.data==="failed"){
                                        realtimeDatabase.ref(`${item_ref}`).ref.child("response")
                                            .set({ code: 406 })
                                        ...
                                        if(dataSnapshot.exists){
                                            transactionDataSnapshot.ref.set({ ...account,balance: account["balance"] +transactionData}, { merge: true })
                                            .then(()=>{
                                                realtimeDatabase.ref(`${item_ref}`).ref.child("response")
                                                    .set({ code: 201 })
                                                resolve(202)
                                            ...
                                            transactionDataSnapshot.ref.set({...account,balance: account["balance"] +transactionData})
                                            .then(()=>{
                                                realtimeDatabase.ref(`${item_ref}`).ref.child("response")
                                                    .set({ code: 201 })
                                                resolve(202)
